...
def classify_query(state: SupportState):
    ...
    - account: Account access, password reset, profile change, login issues
    - general: General questions, features inquiries, information requests
...
def handle_billing(state: SupportState):
    ...
    prompt = f"""
    You are a billing specialist. Handle this customer query: {state["customer_query"]}
    ...
    return {"response": response, "tools_used": ["billing_database", "payment_processor"]}
...
def handle_refund(state: SupportState):
    ...
    Be emapthetic and solution-focused.
    """
...
def handle_general(state: SupportState):
    print("GENERAL HANDLER: Processing general inquiry...")
