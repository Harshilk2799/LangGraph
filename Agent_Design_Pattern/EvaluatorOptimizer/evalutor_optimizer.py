...
def generate_description(state: OptimizationState):
    ...
    if iteration == 1:
        ...
        Requirements:
        - Headline: Catchy an concise (max 10 words)
        - Description: Engaging and information (100-150 words)
        ...
        EVALUATION SCORES:
        - Overall: {state['evaluation_results'].get("overall_score", 0)}/10
        - Clarity: {state['evaluation_results'].get("clarity_score", 0)}/10
        - Persuasuveness: {state['evaluation_results'].get("persuasiveness_score", 0)}/10
        - Audience Fit: {state['evaluation_results'].get("audience_fit_score", 0)}/10

        FEEDBACK TO ADDRESS:
        {state['feedback']}

        CRITICAL: Focus on the specifi weaknesses mentioned. Make targeted improvements to:
        1. Address each point in the feedback 
        2. Maintain the strengths that were working 
        3. Increase scores in weak area 
...
def evaluate_description(state: OptimizationState):
    print(f"\n{'='*70}")
    ...
    - Overall score (average for criteria)
    - Whether it's approved (score >=8)
    - Specific strengths (What's workin well)
    - Specific weakness (What needs improvement)
...
def should_continue(state: OptimizationState)-> Literal["optimizer", "end"]:
    ...
    print(f"\n CONTIUNING: Routing back to optimizer for iteration {state['iteration_count']}")
